...
const isRouteAuthorized = (pathname: string, userRole: string | null | undefined): boolean => {
  ...
  // --- Doctor Leads, Doctor CRM & Webinars (Admin, Executive Admin, Investor Relations) ---
  if (
    ...
    pathname === '/dashboard/webinars' || pathname.startsWith('/dashboard/webinars/')
  ) {
    return role === 'admin' || role === 'executive_admin' || role === 'investor_relations';
  }
  ...
  useEffect(() => {
    ...
    if (!loading && !user && !sessionExpired) {
      ...
      if (path !== '/' && !path.startsWith('/auth')) {
        // Save the current URL to session storage to redirect after login if it's an invite link
