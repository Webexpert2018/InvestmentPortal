      ...
      try {
        ...
        if (userData) {
          ...
          setProfile({
            ...
            notif_sms_security: !!userData.notif_sms_security,
          });
      ...
      <SectionCard>
        ...
        <div className="divide-y divide-[#ECEDEF] p-4">
                ...
                if (user?.twoFactorEnabled) {
                  ...
                  if (confirm('Are you sure you want to disable two-factor authentication? This will make your account less secure.')) {
                    try {
                      ...
                      refreshUser();
                      toast({ title: 'Success', description: '2FA has been disabled', variant: 'success' });
                    } catch (err: any) {
          ...
          <div className="flex items-start justify-between py-3">
            ...
            <Toggle 
              enabled={profile.notif_sms_security === true} 
              onChange={async () => {
                try {
                  const newVal = !profile.notif_sms_security;
                  await apiClient.updateProfile({ notif_sms_security: newVal });
                  setProfile(prev => ({ ...prev, notif_sms_security: newVal }));
          ...
          <div className="py-4">
            <button 
              ...
              className="h-[32px] rounded-full bg-[#FBCB4B] px-5 text-[12px] font-semibold text-[#1F1F1F] disabled:opacity-50 hover:bg-[#F9BF2A] transition-colors"
            >
